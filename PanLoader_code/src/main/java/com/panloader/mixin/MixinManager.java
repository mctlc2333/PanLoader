...
public class MixinManager {
    ...
    private final MixinOrchestrator orchestrator = MixinOrchestrator.getInstance();
    ...
    public void init() {
        ...
        try {
            ...
            if (mixinEnvClass != null) {
                ...
                List<String> orderedConfigs = orchestrator.getOrderedMixinConfigs();
                for (String configPath : orderedConfigs) {
                    registerConfig(env, mixinEnvClass, configPath);
                }

                loadedMixinConfigs.add(CONFIG_FILE);
                loadedMixinConfigs.addAll(orderedConfigs);

                orchestrator.applyToEnvironment(env, mixinEnvClass);
            }
            ...
            System.out.println("[PanLoader] Total Mixin configs loaded: " + loadedMixinConfigs.size());
        } catch (Exception e) {
    ...
    public MixinOrchestrator getOrchestrator() {
        return orchestrator;
    }
